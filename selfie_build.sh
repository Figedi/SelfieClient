...
if [[ $clean = 1 ]] ; then
	...
	if [[ -d "hooks" ]] ; then
		message_info "Removing 'hooks' directory."
		rm -rf hooks
	fi
fi
...
if [[ $init = 1 ]] ; then
	...
	#message_info "Adding WP8 platform..."
...
if [[ $init = 1 ]] || [[ $merge = 1 ]] ; then
	...
	#message_info "Merging WP8 platform customizations..."
	#cp -R platform-merges/wp8/* platforms/wp8/
fi
...
if [[ $init = 1 ]] || [[ $icons = 1 ]] ; then
	...
	if [[ -d "platforms/android" ]] && [[ -d "www/res/icon/android" ]] ; then
	...
	if [[ -d "platforms/ios" ]] && [[ -d "www/res/icon/ios" ]] ; then
...
if [[ $init = 1 ]] || [[ $plugins = 1 ]] ; then
	...
	message_info "Adding Camera Plugin..."
...
if [[ $init = 1 ]] || [[ $update = 1 ]] ; then
	...
	#message_info "Syncing 'www' with WP8 platform..."
	#cordova prepare wp8
fi
